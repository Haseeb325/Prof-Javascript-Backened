...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    // is ma aesa ha k agr upload hogai to  thk ha agr na jui to 1 empty string de de ga
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  const incomingRefreshToken =
    req.cookies.refreshToken || req.body.refreshToken; //  agr koi mobile pe b acces krna chahe
...
const updateUsercoverImage = asyncHandler(async (req, res) => {
  ...
  // ab update kr do
  const user = await User.findByIdAndUpdate(
    // const user ye ham ne refrece ma le lia ye response ma bhejne k liay agr response ni bhejna hota to variable ma store krne ki zarorat ni
    req.user?._id, // ise update kr do
    {
      $set: {
        // ye update kr do
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    {
      $addFields: {
        // mtlb k ye wale sare fields add krne hain
        subscribersCount: {
        ...
        isSubscribed: {
          $cond: {
            // ab ham dekhe ge k jo ham ne subscribers count kiay un ma me mai hun k ni
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [
          ...
          {
            $addFields: {
              // ye additional kam ha wese upr tk to array mile ga saray ka sara lekin frontened ko aur easy krne k liay ham ne array se pehla field mtlb data wala separate le lia
              owner: {
                $first: "$owner",
              },
            },
